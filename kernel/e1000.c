...
e1000_transmit(struct mbuf *m)
{
  ...
  struct mbuf *buf = tx_mbufs[index];
  if (buf != 0){
    mbuffree(buf);      // free last buffer
  }
  ...
  // cmd flags: end of packet, report status
